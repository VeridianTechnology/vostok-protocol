...
const CTAFooter = ({ onRequestBuy, entrySource = "direct" }: CTAFooterProps) => {
  ...
  const isFacebook = entrySource === "facebook";
  const [isRedirecting, setIsRedirecting] = useState(false);
  const [countdown, setCountdown] = useState(3);
  const [isDesktop, setIsDesktop] = useState(false);
  const [isFacebookLoading, setIsFacebookLoading] = useState(false);
  const facebookLoadTimeoutRef = useRef<number | null>(null);
  const [buttonLabel, setButtonLabel] = useState<string>("$44.99");
  ...
  return (
    ...
    >
      ...
      {/* Guarantee badges */}
      <div className="absolute bottom-12 right-5 z-20 flex items-center gap-2 md:bottom-20">
        {/* Book badge */}
